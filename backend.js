...
async function hydrate(){
  ...
  try{
    ...
    db.users=profiles.map(p=>({id:p.id,email:p.id===sessionUser.id?sessionUser.email:'',pseudo:p.pseudo||'Utilisateur',avatar:p.avatar_url||'🙂',siteRole:p.site_role||'user',created:ts(p.created_at)}));
...
modalHtml=function(){
  ...
  if(modal==='signup')return `<div class="modal-wrap"><form class="modal form" onsubmit="signup(event)"><div class="modal-head"><div><span class="eyebrow">Compte général Myown</span><h2>Créer mon profil</h2></div><button type="button" class="close" onclick="closeModal()">×</button></div><div class="field"><label>Pseudo</label><input name="pseudo" required maxlength="30"></div><div class="field"><label>E-mail</label><input name="email" type="email" autocomplete="email" required></div><div class="field"><label>Mot de passe</label><input name="password" type="password" autocomplete="new-password" minlength="8" required></div><div class="field"><label>Avatar (emoji ou URL)</label><input name="avatar" value="🙂"></div><div class="notice">Chaque application conserve ensuite son accès indépendant. Pour Colloc, une demande doit être acceptée par l’admin.</div><button class="btn">Créer le compte</button></form></div>`;
...
render=function(){let body=route==='home'?home():route==='apps'?apps():route==='colloc'?colloc():route==='admin'?admin():route==='profile'?profile():home();document.querySelector('#app').innerHTML=`<div class="shell">${topbar()}${body}${modalHtml()}<div class="demo" style="background:#dcf5df;border-color:#a8d7ae">● Backend sécurisé actif</div></div>`};
...
signup=async function(e){e.preventDefault();const f=new FormData(e.target);const {data,error}=await sb.auth.signUp({email:f.get('email'),password:f.get('password'),options:{emailRedirectTo:'https://ratawax.github.io/Myown/',data:{pseudo:f.get('pseudo'),avatar_url:f.get('avatar')}}});if(error)return err(error);modal=null;if(!data.session){render();toast('Compte créé : vérifie ton e-mail pour confirmer.');}else{await hydrate();toast('Compte créé. Le premier compte devient admin technique.')}};
...
decide=async function(id,status){try{const {error}=await sb.rpc('review_access_request',{request_id:id,new_status:status,decision_note:null});if(error)throw error;await hydrate();toast('Décision enregistrée.')}catch(e){err(e)}};
...
saveProfile=async function(e){e.preventDefault();const f=new FormData(e.target);try{await q('profiles',sb.from('profiles').update({pseudo:f.get('pseudo'),avatar_url:f.get('avatar')}).eq('id',db.current));await hydrate();toast('Profil mis à jour.')}catch(x){err(x)}};
