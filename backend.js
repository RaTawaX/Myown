...
decide=async function(id,status){try{const req=db.requests.find(r=>r.id===id);if(!req)throw new Error('Demande introuvable.');const rpc=await sb.rpc('review_access_request',{request_id:id,new_status:status,decision_note:null});if(rpc.error){const missing=/review_access_request|schema cache|Could not find the function/i.test(rpc.error.message||'');if(!missing)throw rpc.error;await q('access_requests',sb.from('access_requests').update({status,decided_at:new Date().toISOString()}).eq('id',id));if(status==='accepted')await sb.from('app_members').upsert({app_id:collocApp.id,user_id:req.userId,role:'member'},{onConflict:'app_id,user_id'}).then(r=>{if(r.error)throw r.error;});}await hydrate();toast('Décision enregistrée.')}catch(e){err(e)}};
